...
def is_authenticated_CSC_user():
    ...
    if is_authenticated_through_direct_proxy():
        if (
            "samlUserdata" in session
            and len(session.get("samlUserdata", None)) > 0
            and SAML_ATTRIBUTES.get("CSC_username") in session.get("samlUserdata", None)
        ):
            return True
...
def get_user_csc_name():
    ...
    # Authenticated through direct proxy
    if is_authenticated_through_direct_proxy():
        csc_name = session.get("samlUserdata", {}).get(
            SAML_ATTRIBUTES.get("CSC_username", None), False
        )
        if csc_name:
            return csc_name[0]
...
def get_user_email():
    ...
    # Authenticated through direct proxy
    if is_authenticated_through_direct_proxy():
        csc_email = session.get("samlUserdata", {}).get(
            SAML_ATTRIBUTES.get("email", None), False
        )
...
def get_user_firstname():
    ...
    # Authenticated through direct proxy
    if is_authenticated_through_direct_proxy():
        first_name = session.get("samlUserdata", {}).get(
            SAML_ATTRIBUTES.get("first_name", None), False
        )
        if first_name:
            return first_name[0]
...
def get_user_lastname():
    ...
    # Authenticated through direct proxy
    if is_authenticated_through_direct_proxy():
        lastname = session.get("samlUserdata", {}).get(
            SAML_ATTRIBUTES.get("last_name", None), False
        )
        if lastname:
            return lastname[0]
...
def get_user_ida_projects():
    ...
    # Authenticated through direct proxy
    if is_authenticated_through_direct_proxy():
        groups = session.get("samlUserdata", {}).get(
            SAML_ATTRIBUTES.get("idm_groups", None), False
        )
        idaProjects = (
            [group for group in groups if group.startswith("IDA01:")]
            if groups
            else not_found("ida_projects")
        )
    ...
    if is_authenticated_through_fairdata_sso():
        ...
        if session_data.get("services").get("IDA"):
            try:
                user_ida_projects = (
                    session_data.get("services").get("IDA").get("projects")
                )
...
def get_user_home_organization_id():
    ...
    if is_authenticated_through_direct_proxy():
        home_organization_id = session.get("samlUserdata", {}).get(
            SAML_ATTRIBUTES.get("haka_org_id", None), False
        )
    ...
    if is_authenticated_through_fairdata_sso():
        session_data = get_decrypted_sso_session_details()
        user_home_organization_id = (
            session_data.get("authenticated_user").get("organization").get("id")
        )
...
def get_user_home_organization_name():
    ...
    # Authenicated through direct proxy
    if is_authenticated_through_direct_proxy():
        home_organization_name = session.get("samlUserdata", {}).get(
            SAML_ATTRIBUTES.get("haka_org_name", None), False
        )
    ...
    if is_authenticated_through_fairdata_sso():
        session_data = get_decrypted_sso_session_details()
        user_home_organization_name = (
            session_data.get("authenticated_user").get("organization").get("name")
        )
...
def get_user_haka_identifier():
    ...
    haka_id = session.get("samlUserdata", {}).get(
        SAML_ATTRIBUTES.get("haka_id", None), False
    )
