  ...
  render() {
    return (
      <Form>
        <InputContainer>
          <List>
            <Key><Translate content="dataset.project.name" /></Key>
            <Value>{checkDataLang(this.state.project.name)}</Value>
            {this.state.project.identifier && (
              <div>
                <Key><Translate content="dataset.project.identifier" /></Key>
          ...
          {checkNested(this.state.project, 'homepage', 'identifier') && (
            <div>
              <Topic><Translate content="dataset.project.homepage" /></Topic>
              <List>
                <Key><Translate content="dataset.project.homepageUrl" /></Key>
                <Value>
                  <a
                ...
                {this.state.project.homepage.description && (
                  <div>
                    <Key><Translate content="dataset.project.homepageDescr" /></Key>
                    <Value>{checkDataLang(this.state.project.homepage.description)}</Value>
                  </div>
                )}
              </List>
            </div>
          ...
          {checkNested(this.state.project, 'source_organization') && (
            <div>
              <Topic><Translate content="dataset.project.sourceOrg" /></Topic>
              <OrgList>
                {this.state.project.source_organization.map(org => (
                  <dd key={checkDataLang(org.name)}>
                    <Agent
                      lang={this.state.lang}
                      first
                      agent={org}
          ...
          {this.hasFunderInfo() && (
            <div>
              <Topic><Translate content="dataset.project.funding" /></Topic>
              <List>
                {this.state.project.has_funder_identifier && (
                  <div>
                    <Key><Translate content="dataset.project.has_funder_identifier" /></Key>
                    <Value>{this.state.project.has_funder_identifier}</Value>
                  </div>
                )}
                {checkNested(this.state.project, 'funder_type', 'pref_label') && (
                  <div>
                    <Key><Translate content="dataset.project.funder_type" /></Key>
