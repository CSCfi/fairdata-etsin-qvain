    ...
    return selected.map(s => (
      <Fragment key={`${s.id}-${randomStr()}`}>
        <SelectedFilesItem tabIndex="0" active={isInEdit(inEdit, s.identifier)}>
          <FileLabel>
            <FontAwesomeIcon
              icon={s.directoryName ? faFolder : faCopy}
              style={{ marginRight: '8px' }}
            />
            {s.projectIdentifier} / {s.directoryName || s.fileName}
          </FileLabel>
          {s.removed && (
            <Translate component={DeletedLabel} content="qvain.files.deletedLabel" color="error" />
          )}
          <FileButtonsContainer>
            {s.directoryName && existing && isCumulative && canSelectFiles && !deprecated && (
              ...
              >
                <Translate
                  component={RefreshDirectoryButtonText}
                  content="qvain.files.refreshModal.buttons.show"
                />
              </RefreshDirectoryButton>
            )}
    ...
    return (
      <Fragment>
        {canSelectFiles && (
          <>
            <Translate
              ...
              content="qvain.files.selected.title"
            />
            {selected.length === 0 && (
              <Translate tabIndex="0" component="p" content="qvain.files.selected.none" />
            )}
        ...
        <Translate
          tabIndex="0"
          component={SelectedFilesTitle}
          content="qvain.files.existing.title"
        />
        ...
        <Modal
          ...
          isOpen={
            selected.length > 0 &&
            existing.length > 0 &&
            this.state.datasetDuplicationModalHasNotBeenShown === true &&
            !isCumulative
          }
        ...
        >
          <Translate
            component="h3"
            content="qvain.files.notificationNewDatasetWillBeCreated.header"
          />
          <Translate
            component="p"
            content="qvain.files.notificationNewDatasetWillBeCreated.content"
          />
