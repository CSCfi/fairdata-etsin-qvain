...
export class FileSelectorBase extends Component {
  ...
  drawFile = f => {
    const { toggleSelectedFile, setMetadataModalFile, canSelectFiles } = this.props.Stores.Qvain
    ...
    return (
      <li key={f.identifier} style={{ paddingLeft: '20px', display: 'flex', alignItems: 'center' }}>
        <Checkbox
          ...
          type="checkbox"
          onChange={() => toggleSelectedFile(f, !f.selected)}
          style={{ marginRight: '8px' }}
        />

        <FileIcon />
        <label
          htmlFor={`${f.id}Checkbox`}
          style={{ flexGrow: 1, cursor: 'pointer', fontSize: '85%' }}
        >
          {f.fileName}
        </label>
        <FilePickerFileButton
          id={`${f.identifier}-open-metadata-modal`}
          type="button"
          onClick={() => setMetadataModalFile(f)}
        >
  ...
  drawHierarchy = (h, root) => {
    const { toggleSelectedDirectory, canSelectFiles } = this.props.Stores.Qvain
    return (
      <li key={h.identifier} style={{ paddingLeft: '20px' }}>
        ...
        <Checkbox
          ...
          id={`${h.id}Checkbox`}
          type="checkbox"
          onChange={() => toggleSelectedDirectory({ ...h }, !h.selected)}
        />
        <DirectoryIcon />
        {h.directoryName} ({h.original.file_count})
        <ul>
          {h.directories && h.open && h.directories.map(dir => this.drawHierarchy(dir, h))}
  ...
  render() {
    const { hierarchy } = this.props.Stores.Qvain
    return (
...
export const FilePickerFileButton = styled.button`
  background-color: ${props =>
    props.disabled ? props.theme.color.primaryLight : props.theme.color.primary};
  font-size: 0.9em;
  color: #fff;
  height: 22px;
  border-radius: 2px;
  border: solid 1px
    ${props => (props.disabled ? props.theme.color.primaryLight : props.theme.color.primary)};
  text-transform: none;
  display: inline-flex;
  position: relative;
  align-items: center;
