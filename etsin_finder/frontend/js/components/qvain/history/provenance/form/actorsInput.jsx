...
const ActorsInput = ({ Stores, language }) => {
  ...
  const options = [{ value: 'create-actor', label: 'Luo uusi toimija' }, ...Actors.actorOptions.map(
    option => {
  ...
  const createActor = () => {
    ...
    editActor(Actor({ roles: [ROLE.PROVENANCE] }), (freshActor) => Provenances.inEdit.associations.addActorWithId(freshActor.uiid))
  }
    ...
    <Observer>
      {() => (
        <>
          ...
          <ActorsList language={language} Stores={Stores} actors={Provenances.inEdit.associations} items={(Provenances.inEdit.associations || {}).actorOptions} />
          <Translate
...
const CustomOption = ({ children, ...props }) => (
  <>
    ...
    >
      {children}
      {' '}
      {props.value === 'create-actor' && <EditIcon color="primary" />}
    </selectComponents.Option>
  </>
)
