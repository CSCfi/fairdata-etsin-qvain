...
def logout_etsin():
    ...
    if sso_is_enabled:
        ...
        resp.set_cookie(
            "sso_authentication", "", expires=0, domain=formatted_shared_domain
        )
        return resp
...
def saml_attribute_consumer_service_legacy():
    ...
    if host == "30.30.30.30":
        return redirect(
            "https://" + current_app.config.get("SERVER_ETSIN_DOMAIN_NAME") + "/acs/",
            307,
        )
    ...
    if len(errors) == 0 and auth.is_authenticated():
        ...
        if "samlUserdata" in session and len(session.get("samlUserdata", None)) > 0:
            ...
            if logged_in_through == "qvain":
                redirect_url = "https://" + current_app.config.get(
                    "SERVER_QVAIN_DOMAIN_NAME"
                )

            # Redirect for Etsin
            if logged_in_through == "etsin":
                redirect_url = "https://" + current_app.config.get(
                    "SERVER_ETSIN_DOMAIN_NAME"
                )

            if redirect_url:
                if "RelayState" in request.form:
                    redirect_url = urljoin(
                        redirect_url, unquote(request.form.get("RelayState"))
                    )
