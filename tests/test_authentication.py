...
class TestAuth(BaseTest):
    ...
    def test_sso_login_url_lumi_aif_uses_etsin_environment_domain(self, session_lang_en, app):
        ...
        with app.test_request_context():
            ...
            assert (
                query
                == "sso_fake_host/login?service=ETSIN&redirect_url=https%3A%2F%2Flumi-aif.fd-dev.csc.fi&language=en"
            )
    ...
    def test_sso_logout_url_lumi_aif_uses_etsin_environment_domain(
        self, session_lang_en, app
    ):
        """Return SSO logout URL with Lumi-AIF env-aware redirect domain."""
        with app.test_request_context():
            app.config["SERVER_ETSIN_DOMAIN_NAME"] = "etsin.fd-dev.csc.fi"
            ...
            assert (
                query
                == "sso_fake_host/logout?service=ETSIN&redirect_url=https%3A%2F%2Flumi-aif.fd-dev.csc.fi&language=en"
            )
