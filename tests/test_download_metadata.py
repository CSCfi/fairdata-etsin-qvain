...
class TestDownloadMetadataService(BaseTest):
    ...
    def test_preserve_headers(self, unauthd_client, open_catalog_record, requests_mock):
        ...
        matcher = self.get_matcher('<someXml>Hello</someXml>', {
            'Content-Type': 'application/somefileformat',
            'Content-Disposition': 'content-disposition',
            'Content-Length': '24',
        })
        requests_mock.get('https://mock-metax/rest/datasets/1.json', text=matcher, status_code=200)
    ...
    def test_type_datacite_xml(self, unauthd_client, open_catalog_record, requests_mock):
        """Should return metadata.xml for fairdata_datacite format"""
        matcher = self.get_matcher('<someXml>Hello</someXml>', {
            'Content-Type': 'application/xml',
        })
        requests_mock.get('https://mock-metax/rest/datasets/1?dataset_format=fairdata_datacite', text=matcher, status_code=200)
    ...
    def test_type_xml(self, unauthd_client, open_catalog_record, requests_mock):
        ...
        requests_mock.get('https://mock-metax/rest/datasets/1?dataset_format=fairdata', text=matcher, status_code=200)
    ...
    def test_type_json(self, unauthd_client, open_catalog_record, requests_mock):
        ...
        matcher = self.get_matcher('{ "content": "hello world"}', {
    ...
    def test_invalid_format(self, unauthd_client, open_catalog_record, requests_mock):
        ...
        matcher = self.get_matcher({"detail": ["Requested format 'thisformatdoesnotexist' is not available"]}, {
            'Content-Type': 'application/json',
        })
        requests_mock.get('https://mock-metax/rest/datasets/1?dataset_format=thisformatdoesnotexist', json=matcher, status_code=400)
