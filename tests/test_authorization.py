...
class TestUserhasDatasetProject(BaseTest):
    ...
    def test_matching_project(self, app, IDA_projects_ok, requests_mock):
        ...
        with app.app_context():
            requests_mock.get('https://mock-metax/rest/v2/datasets/1/projects', json=['project_x'], status_code=200)
...
class TestUserCanViewDataset(BaseTest):
    ...
    def cr_published(self, monkeypatch):
        """Published dataset"""
        monkeypatch.setattr(cr_service, "get_catalog_record", lambda *x: {'state': 'published'})

    @pytest.fixture
    def cr_draft(self, monkeypatch):
        """Draft dataset"""
        monkeypatch.setattr(cr_service, "get_catalog_record", lambda *x: {'state': 'draft'})

    @pytest.fixture
    def cr_draft_teppo(self, monkeypatch):
        """Draft dataset by teppo_testaaja"""
        monkeypatch.setattr(cr_service, "get_catalog_record",
    ...
    def test_loggedin_view_draft_ok_user(self, flagged_app, user_details, cr_draft_teppo, cr_permissions_empty ):
        """Creator of draft can view it"""
        with flagged_app.test_request_context():
            assert authorization.user_can_view_dataset(1) is True

    def test_loggedin_view_draft_ok_project(self, flagged_app, user_details, IDA_projects_ok, cr_draft, cr_permissions_project_x):
        """Non-creator project members can view draft"""
        with flagged_app.test_request_context():
            assert authorization.user_can_view_dataset(1) is True

    def test_loggedin_view_project_flag_disabled(self, flagged_app, user_details, IDA_projects_ok, cr_draft, cr_permissions_project_x):
        """Non-creator project members can't view draft if flag is disabled"""
        set_flags({'PERMISSIONS.SHARE_PROJECT': False}, flagged_app)
        with flagged_app.test_request_context():
            assert authorization.user_can_view_dataset(1) is False

    def test_loggedin_view_draft_denied(self, flagged_app, user_details, IDA_projects_ok,
                                        cr_draft_otheruser, cr_permissions_project_y):
