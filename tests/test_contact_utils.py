...
class TestContactUtils(ContactTest):
    ...
    def test_get_email_message_body(self, mock_datetime_now, expected_email_body, pref_id, user_email, user_subject, user_body):
        """Test that function returns a correct email body."""
        result_email = create_email_message_body(pref_id, user_email, user_subject, user_body)
    ...
    def test_validate_send_message_request_invalid_email(self, app, expect_log, invalid_user_email, user_body, agent_type_creator):
        """Test that function returns False and warns when invalid email."""
        with app.app_context():
            result = validate_send_message_request(invalid_user_email, user_body, agent_type_creator)
            assert result is False
            expect_log(warnings=["Reply-to email address not formally valid: " + invalid_user_email])

    def test_validate_send_message_request_unknown_agent_type(self, app, expect_log, user_email, user_body, invalid_agent_type):
        """Test that function returns False and warns when invalid agent type."""
        with app.app_context():
            result = validate_send_message_request(user_email, user_body, invalid_agent_type)
            assert result is False
            expect_log(warnings=["Unrecognized agent type"])

    def test_validate_send_message_request_user_body_too_long(self, app, expect_log, user_email, long_user_body, agent_type_creator):
        """Test that function returns False and warns when user body is too long."""
        with app.app_context():
            result = validate_send_message_request(user_email, long_user_body, agent_type_creator)
            assert result is False
            expect_log(warnings=["Body is too long"])

    def test_validate_send_message_request_ok(self, user_email, user_body, agent_type_creator):
        """Test that function returns False and warns when user body is too long."""
        result = validate_send_message_request(user_email, user_body, agent_type_creator)
    ...
    def test_get_email_recipient_addresses_invalid_agent_type(self, app, expect_log, catalog_record):
        ...
        with app.app_context():
            ...
            expect_log(errors=["No email addresses found with given agent type NON_EXISTANT"])
    ...
    def test_get_email_info_with_catalog_record(self, catalog_record):
        ...
        assert result == {'CREATOR': True, 'PUBLISHER': True, 'CONTRIBUTOR': True, 'RIGHTS_HOLDER': True, 'CURATOR': True}
